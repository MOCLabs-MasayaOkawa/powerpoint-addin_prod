...
namespace PowerPointEfficiencyAddin.Services
{
    ...
    public class PowerToolService
    {
        ...
        private string ShowFontSelectionDialog()
        {
            ...
            try
            {
                ...
                {
                    form.Text = "フォント一括統一";
                    form.Size = new System.Drawing.Size(380, 250);
                    ...
                    {
                        Text = "キャンセル",
                        Location = new System.Drawing.Point(265, 170),
                        Size = new System.Drawing.Size(75, 25),
        ...
        public void ConvertTableToTextBoxes()
        {
            ...
            if (tableShapes.Count == 0)
            {
                ErrorHandler.ExecuteSafely(() =>
                {
                    throw new InvalidOperationException("表図形を選択してください。");
                }, "表をテキストボックスに変換");
                return;
            }
            ...
            ComHelper.ExecuteWithComCleanup(() =>
            {
                ...
                foreach (var tableShape in tableShapes)
                {
                    try
                    {
                        ConvertSingleTableToTextBoxes(slide, tableShape.Shape);
                        logger.Debug($"Converted table {tableShape.Name} to text boxes");
                    }
                    catch (Exception ex)
                    {
                        logger.Error(ex, $"Failed to convert table {tableShape.Name}");
                    }
                }
            }, tableShapes.Select(s => s.Shape).ToArray());
        ...
        public void ConvertTextBoxesToTable()
        {
            ...
            if (textBoxShapes.Count < 2)
            {
                ErrorHandler.ExecuteSafely(() =>
                {
                    throw new InvalidOperationException("テキストを含む図形を2つ以上選択してください。");
                }, "テキストボックスを表に変換");
                return;
            }
            ...
            ComHelper.ExecuteWithComCleanup(() =>
            {
                ...
                try
                {
                    ...
                    // 行高統一後に位置を再調整（0.2cm間隔を維持）
                    var spacing = 0.2f * 28.35f; // 0.2cm間隔
                    AdjustPositionsAfterHeightUnification(gridInfo.ShapeGrid, gridInfo.TopLeft.Left, gridInfo.TopLeft.Top, spacing);

                    ConvertGridToTable(slide, gridInfo);
                    logger.Info($"Converted {textBoxShapes.Count} text boxes to table ({gridInfo.Rows}x{gridInfo.Columns}) with unified row heights and 0.2cm spacing");
                }
                catch (Exception ex)
                {
                    logger.Error(ex, "Failed to convert text boxes to table");
                    throw;
                }
            }, textBoxShapes.Select(s => s.Shape).ToArray());
        ...
        /// <param name="tableShape">表図形</param>
        private void ConvertSingleTableToTextBoxes(PowerPoint.Slide slide, PowerPoint.Shape tableShape)
        {
            ...
            {
                // 各行・列のサイズを取得
                var columnWidths = new float[tableColumns];
                var rowHeights = new float[tableRows];

                // 列幅を取得（元のまま保持）
                for (int col = 1; col <= tableColumns; col++)
                {
                    columnWidths[col - 1] = table.Columns[col].Width;
                }
                ...
                for (int row = 1; row <= tableRows; row++)
                {
                    ...
                    // 同じ行内の全セルの高さを確認して最大値を取得
                    for (int col = 1; col <= tableColumns; col++)
                    {
                        try
                        {
                            var cell = table.Cell(row, col);
                            var cellHeight = cell.Shape.Height;
                            maxRowHeight = Math.Max(maxRowHeight, cellHeight);
                        }
                        catch (Exception ex)
                        {
                            logger.Warn(ex, $"Failed to get height for cell [{row},{col}], using row height");
                            maxRowHeight = Math.Max(maxRowHeight, table.Rows[row].Height);
                        }
                    }

                    rowHeights[row - 1] = maxRowHeight;
                    logger.Debug($"Row {row} unified height: {maxRowHeight:F1}pt");
                }

                // 各セルをテキストボックスに変換（左上基準でグリッド配置）
                ...
                for (int row = 1; row <= tableRows; row++)
                {
                    ...
                    {
                        var cell = table.Cell(row, col);
                        var cellWidth = columnWidths[col - 1];
                        var cellHeight = rowHeights[row - 1];
                        ...
                        logger.Debug($"Created text box for cell [{row},{col}] at ({currentLeft:F1}, {currentTop:F1}) size {cellWidth:F1}x{cellHeight:F1}");

                        currentLeft += cellWidth + spacing;
                    }

                    currentTop += rowHeights[row - 1] + spacing;
                }
            ...
            {
                logger.Error(ex, "Failed to convert single table to text boxes");
                throw;
            }
        }
        ...
        private void ConvertGridToTable(PowerPoint.Slide slide, GridInfo gridInfo)
        {
            try
            {
                var tableLeft = gridInfo.TopLeft.Left;
                var tableTop = gridInfo.TopLeft.Top;
                ...
                // 表を作成
                var table = slide.Shapes.AddTable(gridInfo.Rows, gridInfo.Columns,
                    tableLeft, tableTop, totalWidth, totalHeight);

                // 各列の幅を設定
                for (int col = 0; col < gridInfo.Columns; col++)
                {
                    table.Table.Columns[col + 1].Width = columnWidths[col];
                }

                // 各行の高さを設定
                for (int row = 0; row < gridInfo.Rows; row++)
                {
                    table.Table.Rows[row + 1].Height = rowHeights[row];
                }
                ...
                for (int row = 0; row < gridInfo.Rows; row++)
                {
                    for (int col = 0; col < gridInfo.Columns; col++)
                    {
                        var sourceShape = gridInfo.ShapeGrid[row][col];
                        var cell = table.Table.Cell(row + 1, col + 1);

                        if (!string.IsNullOrEmpty(sourceShape.Text))
                        {
                            cell.Shape.TextFrame.TextRange.Text = sourceShape.Text;
                        }

                        if (sourceShape.HasTextFrame)
                        {
                            try
                            {
                                var sourceTextRange = sourceShape.Shape.TextFrame.TextRange;
                                var targetTextRange = cell.Shape.TextFrame.TextRange;
                            ...
                            {
                                logger.Warn(ex, $"Failed to copy text format for cell [{row},{col}]");
                            }
                        }
                    }
                }
                ...
                // 元の図形を削除
                foreach (var shapeRow in gridInfo.ShapeGrid)
                {
                    foreach (var shape in shapeRow)
                    {
                        try
                        {
                            shape.Shape.Delete();
                        ...
                        {
                            logger.Warn(ex, $"Failed to delete shape {shape.Name}");
                        }
                    }
                }

                table.Select();
                logger.Info($"Successfully created table ({gridInfo.Rows}x{gridInfo.Columns}) with original column widths and unified row heights");
            }
            catch (Exception ex)
            {
                logger.Error(ex, "Failed to convert grid to table");
                throw;
            }
        }
        ...
        private void CopyCellFormat(PowerPoint.Shape sourceCell, PowerPoint.Shape targetTextBox)
        {
            try
            {
                if (sourceCell.Fill.Type != MsoFillType.msoFillMixed)
                {
                ...
                if (sourceCell.Line.Visible == MsoTriState.msoTrue)
                {
                    ...
                    targetTextBox.Line.DashStyle = sourceCell.Line.DashStyle;
                }
            }
            catch (Exception ex)
            {
        ...
        public void AddMatrixRowSeparators()
        {
            ...
            // 線設定ダイアログを表示
            UI.LineSeparatorDialog dialog = null;
            try
            {
                dialog = new UI.LineSeparatorDialog();
        ...
        private void RecreateRowSeparators(List<PowerPoint.Shape> existingSeparators, List<SeparatorLinePosition> newPositions, PowerPoint.Slide slide)
        {
            try
            {
                ...
                if (existingSeparators.Count > 0)
                {
                    ...
                    lineColor = System.Drawing.ColorTranslator.FromOle((int)firstSeparator.Line.ForeColor.RGB);
                }
            ...
            try
            {
                ...
                for (int i = 0; i < separatorPositions.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        line.Line.Weight = lineWeight;
                        line.Line.DashStyle = lineStyle;
                        line.Line.ForeColor.RGB = System.Drawing.ColorTranslator.ToOle(lineColor);
                        line.Line.Visible = MsoTriState.msoTrue;
                        line.Name = $"RowSeparator_{i + 1}";
            ...
            try
            {
                ...
                if (newPositions.Count != separatorShapes.Count)
                {
                    ...
                    CreateRowSeparatorLines(slide, null, MsoLineDashStyle.msoLineSolid, 1.0f,
                        System.Drawing.Color.Black, newPositions);
                    return;
                }
        ...
        private (List<ShapeInfo> matrixShapes, List<ShapeInfo> targetShapes) SeparateMatrixAndTargetShapes(List<ShapeInfo> selectedShapes)
        {
            ...
            {
                Microsoft.Office.Interop.PowerPoint.Shape shp = si.Shape;
        ...
        private static bool IsRectLikeAutoShape(Microsoft.Office.Interop.PowerPoint.Shape shp)
        {
            return shp.Type == MsoShapeType.msoAutoShape
                   && RectLikeAutoShapes.Contains(shp.AutoShapeType);
        }

        private static bool IsMatrixPlaceholder(Microsoft.Office.Interop.PowerPoint.Shape shp)
        {
            if (shp.Type != MsoShapeType.msoPlaceholder) return false;
            var pt = shp.PlaceholderFormat.Type; // PpPlaceholderType
            return pt == Microsoft.Office.Interop.PowerPoint.PpPlaceholderType.ppPlaceholderBody
                || pt == Microsoft.Office.Interop.PowerPoint.PpPlaceholderType.ppPlaceholderTitle
                || pt == Microsoft.Office.Interop.PowerPoint.PpPlaceholderType.ppPlaceholderTable;
        }
        ...
        public void MatrixTuner()
        {
            ...
            UI.MatrixTunerDialog dialog = null;
            try
            {
                // AutoFitを無効化
                DisableAutoFitForShapes(rectangularShapes);

                dialog = new UI.MatrixTunerDialog(rectangularShapes, gridInfo);
                var dialogResult = dialog.ShowDialog();
        ...
        /// <summary>
        /// グリッド配置された図形の行内高さを統一します
        /// </summary>
        /// <param name="shapeGrid">行・列に整理された図形グリッド</param>
        private void UnifyRowHeightsInGrid(List<List<ShapeInfo>> shapeGrid)
        {
            try
            {
                for (int row = 0; row < shapeGrid.Count; row++)
                {
                    var rowShapes = shapeGrid[row];
                    if (rowShapes.Count == 0) continue;

                    var maxHeight = rowShapes.Max(s => s.Height);

                    foreach (var shape in rowShapes)
                    {
                        if (Math.Abs(shape.Height - maxHeight) > 0.1f)
                        {
                            shape.Shape.Height = maxHeight;
                            shape.UpdateDimensions();
                            logger.Debug($"Unified height of {shape.Name} in row {row + 1} to {maxHeight:F1}pt");
                        }
                    }
                }
            }
            ...
            {
                logger.Error(ex, "Failed to unify row heights in grid");
            }
        }

        /// <summary>
        /// 作成された図形リストをグリッド構造に変換します
        /// </summary>
        /// <param name="shapes">図形リスト</param>
        /// <param name="rows">行数</param>
        /// <param name="columns">列数</param>
        /// <returns>グリッド構造の図形リスト</returns>
        private List<List<ShapeInfo>> ConvertToShapeGrid(List<PowerPoint.Shape> shapes, int rows, int columns)
        {
            ...
            {
                var rowShapes = new List<ShapeInfo>();
                for (int col = 0; col < columns; col++)
                {
                    var index = row * columns + col;
                    if (index < shapes.Count)
                    {
                        rowShapes.Add(new ShapeInfo(shapes[index], index));
                    }
                }
                grid.Add(rowShapes);
            }
        ...
        /// <summary>
        /// 行高統一後に位置を再調整します（0.2cm間隔を維持）
        /// </summary>
        /// <param name="shapeGrid">図形グリッド</param>
        /// <param name="baseLeft">基準左位置</param>
        /// <param name="baseTop">基準上位置</param>
        /// <param name="spacing">間隔</param>
        private void AdjustPositionsAfterHeightUnification(List<List<ShapeInfo>> shapeGrid, float baseLeft, float baseTop, float spacing)
        {
            try
            {
                ...
                for (int row = 0; row < shapeGrid.Count; row++)
                {
                    ...
                    var currentLeft = baseLeft;
                    var rowHeight = rowShapes.Count > 0 ? rowShapes[0].Height : 0f;

                    for (int col = 0; col < rowShapes.Count; col++)
                    {
                        var shape = rowShapes[col];

                        shape.Shape.Left = currentLeft;
                        shape.Shape.Top = currentTop;
                        shape.UpdateDimensions();

                        logger.Debug($"Readjusted position: [{row},{col}] to ({currentLeft:F1}, {currentTop:F1})");
                ...
                logger.Info("Positions readjusted after height unification to maintain 0.2cm spacing");
            }
            catch (Exception ex)
            {
                logger.Error(ex, "Failed to adjust positions after height unification");
            }
        }
        ...
        #region 新機能グループ - 同色・同サイズ選択、透過率調整
            ...
            var shapeCreationCommands = new HashSet<string>
    {
        ...
        public void ShowShapeStyleDialog()
        {
            ...
            try
            {
                using (var dialog = new UI.ShapeStyleDialog())
                {
        ...
        private void ApplyShapeStyle(PowerPoint.Shape shape, string commandName)
        {
            ...
            {
                // 現在の設定を読み込み
                var settings = Services.SettingsService.Instance.LoadShapeStyleSettings();
        ...
        private void ApplyFontColorStyle(PowerPoint.Shape shape, Models.ShapeStyleSettings settings)
        {
            if (shape == null) return;
        ...
        /// <param name="settings">スタイル設定</param>
        /// <param name="commandName">コマンド名</param>
        private void ApplyFillStyle(PowerPoint.Shape shape, Models.ShapeStyleSettings settings, string commandName)
        {
            try
            {
                // 線系図形は塗りつぶし不要
        ...
        /// <param name="settings">スタイル設定</param>
        /// <param name="commandName">コマンド名</param>
        private void ApplyLineStyle(PowerPoint.Shape shape, Models.ShapeStyleSettings settings, string commandName)
        {
            try
            {
                if (shape.Line != null)

...
namespace PowerPointEfficiencyAddin.Services
{
    ...
    public class AlignmentService
    {
        ...
        {
            using (var dialog = new SpacingAdjustmentDialog("垂直間隔調整", currentSpacing, shapes, true))
            {
        ...
        {
            using (var dialog = new SpacingAdjustmentDialog("水平間隔調整", currentSpacing, shapes, false))
            {
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    return dialog.GetSettings();
